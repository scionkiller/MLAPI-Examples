...
namespace Alpaca.Serialization
{
    ...
    public class BitWriter
    {
        ...
        public void WriteObjectPacked(object value)
        {
            ...
            {
                WriteQuaternion((Quaternion)value, 3);
                return;
            }
            ...
            else if (value is GameObject) 
            {
                ...
                Entity networkedObject = ((GameObject)value).GetComponent<Entity>();
                if(networkedObject == null) 
                {
                    throw new ArgumentException("BitWriter cannot write GameObject types that does not has a Entity component attached. GameObject: " + ((GameObject)value).name);
                } 
                else 
                {
                    WriteUInt32Packed(networkedObject.GetId());
                }
                return;
            }
        ...
        public void WriteQuaternion(Quaternion rotation, int bytesPerAngle)
        {
