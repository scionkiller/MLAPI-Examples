...
{
	// Accessing coordinates of a CurvePoint at infinity causes undocumented behaviour
	internal class CurvePoint
	{
		readonly bool _atInfinity;
		...
		public bool AtInfinity() { return _atInfinity; }

		public override string ToString()
		{
			return _atInfinity ? "(POINT_AT_INFINITY)" : "(" + X + ", " + Y + ")";
		}
	}
	...
	class EllipticCurve
	{
		...
		readonly BigInteger a, b, modulo;
		readonly CurveType type;

		public EllipticCurve( BigInteger a, BigInteger b, BigInteger modulo, CurveType type = CurveType.Weierstrass )
		{
			if(  ( type == CurveType.Weierstrass && (4 * a * a * a) + (27 * b * b) == 0 ) // Unfavourable Weierstrass curves
			  || ( type == CurveType.Montgomery && b * (a * a - 4) == 0 )				  // Unfavourable Montgomery curves
			  )
			{ 
				throw new System.Exception("Unfavourable curve");
			}
			this.a = a;
			this.b = b;
			this.modulo = modulo;
		...
		public CurvePoint Add(CurvePoint p1, CurvePoint p2)
		{
			...
			// Special cases
			if     ( p1.AtInfinity() && p2.AtInfinity() ) { return new CurvePoint(); }
			else if( p1.AtInfinity() )					  { return p2; }
			else if( p2.AtInfinity() )					  { return p1; }
			else if(  p1.X == p2.X 
				   && p1.Y == Inverse(p2).Y
				   )
			{
		...
		{
			if( scalar <= 0    ) { throw new System.Exception( "Cannot multiply by a scalar which is <= 0" ); }
			if( p.AtInfinity() ) { return new CurvePoint(); }
		...
		{
			BigInteger r = BigInteger.Abs(x) > m ? x % m : x;
			return r < 0 ? r + m : r;
		}
