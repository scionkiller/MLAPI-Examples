...
namespace Alpaca
{
	...
	public class TrackedObject : MonoBehaviour
	{
		...
		private int maxPoints
		{
			get
			{
				return (int)(AlpacaNetwork.GetSingleton().config.SecondsHistory / (1f / AlpacaNetwork.GetSingleton().config.EventTickrate));
			}
		}
		...
		internal void ReverseTransform(float secondsAgo)
		{
			...
			float currentTime = AlpacaNetwork.GetSingleton().NetworkTime;
		...
		internal void AddFrame()
		{
			...
			FrameData.Add(AlpacaNetwork.GetSingleton().NetworkTime, new TrackedPointData()
			{
