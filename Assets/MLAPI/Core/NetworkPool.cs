...
namespace MLAPI.Internal
{
    internal class NetworkPool
    {
        ...
        internal NetworkPool(int prefabId, uint size, ushort poolIndex)
        {
            ...
            {
                GameObject go = MonoBehaviour.Instantiate(NetworkingManager.GetSingleton().config.NetworkedPrefabs[prefabId].prefab, Vector3.zero, Quaternion.identity) as GameObject;
